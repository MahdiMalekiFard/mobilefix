...
<div class="bg-slate-50 dark:bg-slate-900 flex flex-col">
    ...
    <div class="bg-gradient-to-br from-indigo-600 via-purple-600 to-indigo-800 dark:from-indigo-700 dark:via-purple-700 dark:to-indigo-900 rounded-3xl p-6 md:p-8 mb-6 text-white shadow-xl mx-4 mt-4">
        <div class="flex flex-col md:flex-row items-center justify-between gap-4 md:gap-6">
            <div class="flex items-center gap-4 md:gap-6 text-center md:text-left">
                ...
                <div>
                    <h3 class="text-2xl md:text-3xl font-bold mb-2">Lieferadresse auswählen</h3>
                    <p class="text-indigo-100 dark:text-indigo-200 text-base md:text-lg">Wählen Sie aus, wohin Ihr Gerät geliefert werden soll</p>
                </div>
            </div>
            ...
            @if($selectedAddress)
                ...
                <div class="flex flex-col sm:flex-row gap-3">
                    <button wire:click="continueToPayment" 
                        ...
                        <span class="relative z-10">Weiter zur Zahlung</span>
    ...
    <div class="px-4 flex-1 flex flex-col">
        @if(empty($userAddresses))
            <!-- Empty State -->
            <div class="bg-white dark:bg-slate-800 rounded-3xl p-8 md:p-12 text-center shadow-xl flex-1 flex flex-col items-center justify-center">
                <div class="w-20 h-20 md:w-24 md:h-24 bg-gradient-to-br from-blue-100 to-blue-200 dark:from-blue-900/50 dark:to-blue-800/50 rounded-full flex items-center justify-center mx-auto mb-4 md:mb-6">
                    <i class="fas fa-map-marker-alt text-2xl md:text-3xl text-blue-600 dark:text-blue-400"></i>
                </div>
                <h4 class="text-xl md:text-2xl font-semibold text-gray-900 dark:text-gray-100 mb-2 md:mb-3">Keine Adressen gefunden</h4>
                <p class="text-gray-700 dark:text-gray-300 text-base md:text-lg mb-6 md:mb-8 max-w-md mx-auto">Sie müssen eine Lieferadresse hinzufügen, bevor Sie mit Ihrer Bestellung fortfahren können.</p>
        ...
        @else
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6 mb-6 flex-1">
                @foreach($userAddresses as $address)
                    <div class="group">
                        <div class="bg-white dark:bg-slate-800 rounded-2xl border-2 {{ $selectedAddress && $selectedAddress->id == $address['id'] ? 'border-emerald-400 dark:border-emerald-500 bg-gradient-to-br from-emerald-50 to-green-50 dark:from-emerald-900/20 dark:to-green-900/20 shadow-lg shadow-emerald-200/50 dark:shadow-emerald-900/20' : 'border-slate-200 dark:border-slate-700 hover:border-indigo-300 dark:hover:border-indigo-600' }} p-4 md:p-6 cursor-pointer transition-all duration-300 hover:-translate-y-2 hover:shadow-xl relative overflow-hidden h-full flex flex-col"
                            ...
                            <div class="absolute top-3 md:top-4 right-3 md:right-4 {{ $selectedAddress && $selectedAddress->id == $address['id'] ? 'opacity-100 scale-100' : 'opacity-0 scale-50' }} transition-all duration-300">
                                <div class="w-6 h-6 md:w-8 md:h-8 bg-emerald-500 dark:bg-emerald-600 rounded-full flex items-center justify-center shadow-lg">
                            ...
                            @if($address['is_default'])
                                <div class="absolute top-3 md:top-4 left-3 md:left-4">
                                    <span class="bg-gradient-to-r from-indigo-600 to-purple-600 dark:from-indigo-700 dark:to-purple-700 text-white px-2 md:px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wide shadow-md">
                                        Standard
                                    </span>
                                </div>
                            @endif
                            ...
                            <div class="pt-6 md:pt-8 flex-1 flex flex-col">
                                ...
                                <div class="flex items-center gap-3 md:gap-4 mb-3 md:mb-4">
                                    <div class="w-10 h-10 md:w-12 md:h-12 bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-700 dark:to-slate-600 rounded-xl flex items-center justify-center">
                                        <i class="fas fa-home text-gray-600 dark:text-gray-300 text-base md:text-lg"></i>
                                    </div>
                                    <h5 class="text-lg md:text-xl font-semibold text-gray-900 dark:text-gray-100">{{ $address['title'] }}</h5>
                                </div>
                                
                                <!-- Address Content -->
                                <div class="mb-4 md:mb-6 flex-1">
                                    <p class="text-gray-700 dark:text-gray-300 leading-relaxed text-sm md:text-base">{{ $address['address'] }}</p>
                                </div>

                                <!-- Address Footer -->
                                <div class="flex items-center justify-between mt-auto">
                                    <span class="text-gray-600 dark:text-gray-400 text-xs md:text-sm font-medium">Klicken zum Auswählen</span>
                                    <div class="w-6 h-6 md:w-8 md:h-8 bg-slate-100 dark:bg-slate-700 rounded-full flex items-center justify-center group-hover:bg-indigo-600 dark:group-hover:bg-indigo-500 group-hover:text-white transition-all duration-300 group-hover:translate-x-1">
                                        <i class="fas fa-arrow-right text-xs md:text-sm"></i>
                                    </div>
                                </div>
                            </div>
                            ...
                            <!-- Hover Effect -->
                            <div class="absolute inset-0 bg-gradient-to-br from-indigo-600/5 to-purple-600/5 dark:from-indigo-400/5 dark:to-purple-400/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"></div>
                        </div>
                    </div>
                @endforeach
                ...
                <div class="group">
                    <div class="bg-white dark:bg-slate-800 border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-2xl p-4 md:p-6 cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:border-indigo-400 dark:hover:border-indigo-500 hover:bg-gradient-to-br hover:from-slate-50 hover:to-indigo-50 dark:hover:from-slate-700/50 dark:hover:to-indigo-900/20 h-full flex items-center justify-center"
                        ...
                        <div class="text-center">
                            ...
                            <h5 class="text-base md:text-lg font-semibold text-gray-900 dark:text-gray-100 mb-1 md:mb-2">Neue Adresse hinzufügen</h5>
                            <p class="text-gray-700 dark:text-gray-300 text-sm md:text-base">Eine neue Lieferadresse erstellen</p>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <span class="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-indigo-50 dark:bg-indigo-500/10">
                        ...
                        <div>
                            ...
                            <p :id="$id('address-modal-desc')" class="text-sm text-slate-500 dark:text-slate-400">
                                Speichern Sie sie in Ihrem Profil für schnelleres Bezahlen.
                            </p>
                        </div>
                    </div>
                ...
                >
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            placeholder="Straße, Gebäude, Wohnung..."
                    ...
                    <div class="flex items-center justify-end gap-3 pt-2">
                        ...
                        >
                            <span wire:loading.remove wire:target="createAddress">Adresse speichern</span>
